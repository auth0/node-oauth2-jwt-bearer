...
const customTokenLocation = "custom-token-location";
...
const customTokenLocationHandler = (req: IncomingMessage, res: ServerResponse) => {
  let query: Record<string, string>;
  new URL(req.url as string, 'http://localhost').searchParams.forEach(
    (val, key) => {
      query = query || {};
...
describe('get-token', () => {
  ...
  describe('when using the default token locations', () => {
    beforeEach(async () => {
    ...
    it('should succeed to get the token from the query string for non-default token location', async () => {
      await expect(
        got(url, {
          resolveBodyOnly: true,
          searchParams: { access_token: 'token' },
  ...
  describe('when using a custom token location', () => {
    beforeEach(async () => {
      server = createServer(customTokenLocationHandler);
      url = await start(server);
    });
    ...
    it('should get the token from the request payload', async () => {
      await expect(
        got(url, {
          resolveBodyOnly: true,
          method: 'POST',
          form: { [customTokenLocation]: 'token' },
        })
      ).resolves.toEqual('token');
    });
    ...
    it('should get the token from the header', async () => {
      await expect(
        got(url, {
          resolveBodyOnly: true,
          headers: {
            [customTokenLocation]: 'Bearer token',
          },
        })
      ).resolves.toEqual('token');
    });
