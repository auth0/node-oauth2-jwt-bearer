...
describe('tokenVerifier / shouldVerifyDPoP', () => {
  ...
  it('"allowed" mode | returns true if the access token has cnf.jkt (DPoP-bound token)', () => {
    const verifier = makeVerifier({
      dpop: { enabled: true, required: false },
    });
...
describe('tokenVerifier / applyAuthChallenges', () => {
  ...
  it('"allowed" mode | returns Bearer + DPoP challenges when no auth scheme and token was from query/body', () => {
  ...
  it('"allowed" mode | only Bearer challenge has error fields when scheme is unknown but token is from query/body (UnauthorizedError)', async () => {
    const verifier = makeVerifier({
      dpop: { enabled: true, required: false },
      query: { access_token: 'abc.def.ghi' },
    });
  ...
  it('"allowed" mode | only Bearer challenge has error fields when scheme is unknown but token is from query/body (InvalidTokenError)', async () => {
    const verifier = makeVerifier({
      dpop: { enabled: true, required: false },
      query: { access_token: 'abc.def.ghi' },
    });
...
describe('tokenVerifier / verify', () => {
  ...
  beforeEach(() => {
    ...
    if (!(dpopVerifier.assertDPoPRequest as any).restore) {
      sinon.stub(dpopVerifier, 'assertDPoPRequest');
    }
  });
  ...
  it('"required" mode | calls `assertDPoPRequest` if DPoP is required', async () => {
    (dpopVerifier.verifyDPoP as sinon.SinonSpy).restore();
    sinon.stub(dpopVerifier, 'verifyDPoP').resolves();
    ...
    await verifier.verify();
    expect((dpopVerifier.assertDPoPRequest as sinon.SinonStub).calledOnce).toBe(
      true
    );
  });
